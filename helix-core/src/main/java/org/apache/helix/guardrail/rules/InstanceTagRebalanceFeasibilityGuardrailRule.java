 ...
 * <p>
 * <b>Behavior.</b> Opt-in per cluster
 * ({@link ClusterConfig#setInstanceTagRebalanceGuardrailEnabled(boolean)}, disabled by default); when
 * disabled the rule returns before any simulation or ZK read. The verdict is overridable with
...
public class InstanceTagRebalanceFeasibilityGuardrailRule implements GuardrailRule {
  ...
  public ValidationResult validate(GuardrailContext context) {
    ...
    if (!clusterConfig.isInstanceTagRebalanceGuardrailEnabled()) {
      // Opt-in per cluster and disabled by default. Returning here (before the WAGED what-if and the
      // fail-closed reads below) keeps the rule a no-op for every caller until a single ClusterConfig
      // change enables it.
      return ValidationResult.feasible();
    }
