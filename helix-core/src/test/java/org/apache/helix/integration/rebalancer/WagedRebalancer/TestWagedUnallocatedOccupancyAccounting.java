 ...
 * bug).
 */
public class TestWagedUnallocatedOccupancyAccounting extends ZkTestBase {
  private static final Logger LOG =
      LoggerFactory.getLogger(TestWagedUnallocatedOccupancyAccounting.class);
   ...
   */
  @Test
  public void testPartitionRelocatesOffInstanceHoldingUnaccountedReplicas() throws Exception {
    ...
    for (String s : _spareNodes) {
      ...
      // The spares also carry CHI_TAG so relocating the victim is genuinely possible. Without
      // eligible spare capacity this test could not tell a broken rebalancer apart from a cluster
      // that simply had nowhere to put the partition.
    ...
    // Without the fix the rebalancer places both chi partitions on the poisoned nodes -- they look
    // free to it because the wedged INDEX_DOWNLOADED occupants are in a state it does not count --
    // and the capacity check then prunes them, leaving chi with zero active replicas. With the fix
    // the rebalancer sees those nodes as full and relocates chi to a spare that has room.
    boolean chiRecovered = TestHelper.verify(() -> flatExternalView(CHI_RESOURCE).values().stream()
        .filter(TestWagedUnallocatedOccupancyAccounting::isActive).count() >= 1, 60_000);
    ...
    Assert.assertTrue(chiRecovered,
        "chi should reach MIN_ACTIVE by relocating onto a tag-eligible spare with free capacity. "
            + "Actual ExternalView=" + chiExternalView + " best-possible=" + chiBestPossible);
  }
