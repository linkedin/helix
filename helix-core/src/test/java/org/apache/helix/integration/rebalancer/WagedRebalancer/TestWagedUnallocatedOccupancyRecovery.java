 ...
 * bug).
 */
public class TestWagedUnallocatedOccupancyRecovery extends ZkTestBase {
  private static final Logger LOG =
      LoggerFactory.getLogger(TestWagedUnallocatedOccupancyRecovery.class);
  ...
  private static final String AFTER_RESOURCE = "after"; // created once the wedge clears

  private static final int START_PORT = 13100;

  // Blocks INDEX_DOWNLOADED->CAUGHT_UP (catch-up) and INDEX_DOWNLOADED->OFFLINE (drop) so replicas
  // wedge in INDEX_DOWNLOADED and cannot vacate. Unlike the accounting test this one releases the
  // latch mid-test: the point here is what happens after the wedge clears.
  ...
  /**
   * Steering placement away from a physically full instance is only half a fix. The other half is
   * letting go: once the wedged replicas finally drain, the instance has to become an ordinary
   * placement candidate again. If the accounting leaked -- if occupancy were recorded but never
   * released -- the instance would be shunned permanently, and the fix would have traded a
   * non-converging cluster for a silently shrinking one. Nothing else in this change's test suite
   * exercises the release path, so it is asserted end to end here.
   */
  @Test
  public void testInstanceIsUsableAgainAfterWedgedReplicasDrain() throws Exception {
    ...
    }, 30_000), "Phase 1 setup: `stuck` must wedge 4 replicas on the poisoned nodes. Actual="
        + flatExternalView(STUCK_RESOURCE));

    // ---------- Phase 2: the victim routes around the physically full nodes ----------
    ...
    for (String spare : _spareNodes) {
      _gSetupTool.getClusterManagementTool().addInstanceTag(CLUSTER_NAME, spare, POISON_TAG);
      _gSetupTool.getClusterManagementTool().addInstanceTag(CLUSTER_NAME, spare, CHI_TAG);
    }
    createWagedResource(CHI_RESOURCE, 2 /*partitions*/, 1 /*replica*/, CHI_TAG);

    Assert.assertTrue(TestHelper.verify(() -> flatExternalView(CHI_RESOURCE).values().stream()
        .filter(TestWagedUnallocatedOccupancyRecovery::isActive).count() >= 1, 60_000),
        "Phase 2 setup: `chi` must get placed somewhere it can run. Actual="
            + flatExternalView(CHI_RESOURCE));

    // ---------- Phase 3: unwedge, and require the occupancy to actually drain ----------
    BLOCK.countDown();

    Assert.assertTrue(TestHelper.verify(() -> flatExternalView(STUCK_RESOURCE).keySet().stream()
        .noneMatch(k -> _poisonNodes.contains(instanceOf(k))), 60_000),
        "once the drop transition is unblocked the wedged replicas must leave the poisoned nodes, "
            + "otherwise the rest of this test proves nothing. Actual="
            + flatExternalView(STUCK_RESOURCE));

    // ---------- Phase 4: the previously wedged nodes must attract work again ----------
    // Deliberately more partitions than the spares have room for: the spares hold `stuck` and
    // `chi` already, so this resource can only be placed in full if the recovered nodes are back
    // in the candidate pool. A fix that never let go would leave partitions unplaced here.
    createWagedResource(AFTER_RESOURCE, 4 /*partitions*/, 1 /*replica*/, CHI_TAG);

    boolean allPlaced = TestHelper.verify(() -> flatExternalView(AFTER_RESOURCE).values().stream()
        .filter(TestWagedUnallocatedOccupancyRecovery::isActive).count() == 4, 90_000);

    Map<String, String> afterEv = flatExternalView(AFTER_RESOURCE);
    long onRecoveredNodes = afterEv.keySet().stream()
        .filter(k -> _poisonNodes.contains(instanceOf(k))).count();

    Assert.assertTrue(allPlaced,
        "every partition of a resource created after the wedge cleared must be placed; if the "
            + "recovered nodes were still being treated as occupied there would not be enough room. "
            + "Actual ExternalView=" + afterEv + " best-possible=" + flatBestPossible(AFTER_RESOURCE));
    Assert.assertTrue(onRecoveredNodes > 0,
        "the previously wedged nodes must be usable again, not permanently shunned. Actual "
            + "ExternalView=" + afterEv);
  }
  ...
  public void afterClass() throws Exception {
    BLOCK.countDown(); // no-op if the test already released it; guards early failures
    if (_controller != null) {
