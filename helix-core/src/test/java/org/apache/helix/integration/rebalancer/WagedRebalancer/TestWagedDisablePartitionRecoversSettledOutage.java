 ...
 */
public class TestWagedDisablePartitionRecoversSettledOutage extends ZkTestBase {
  private static final Logger LOG =
      LoggerFactory.getLogger(TestWagedDisablePartitionRecoversSettledOutage.class);
  ...
  public void testEnablingTheFlagAfterTheOutageHasSettled() throws Exception {
    ...
    long activeAtSettle = flatExternalView(CHI_RESOURCE).values().stream()
        .filter(TestWagedDisablePartitionRecoversSettledOutage::isActive).count();
    ...
    // ---------- Phase 4: the Tier 0 runbook remedy, no new code involved ----------
    // Exclude the physically-full poisoned nodes for chi via the per-instance disabled-partition
    // list that Helix already has. ReplicaActivateConstraint is a HARD constraint reading this
    // from InstanceConfig, so it applies in the baseline scope too -- which is what makes an
    // already-settled placement eligible to move at all.
    ...
    System.out.println("[SETTLED] >>> chi partitions DISABLED on poison nodes at " + new Date());
    ...
    boolean recovered = TestHelper.verify(() -> flatExternalView(CHI_RESOURCE).values().stream()
        .filter(TestWagedDisablePartitionRecoversSettledOutage::isActive).count() >= 1, 120_000);

    System.out.println("[SETTLED] chi active after DISABLE = "
        + flatExternalView(CHI_RESOURCE).values().stream()
            .filter(TestWagedDisablePartitionRecoversSettledOutage::isActive).count());
    System.out.println("[SETTLED] chi best-possible after DISABLE = " + flatBestPossible(CHI_RESOURCE));
    System.out.println("[SETTLED] chi external view after DISABLE = " + flatExternalView(CHI_RESOURCE));
    System.out.println("[SETTLED] RESULT recovered=" + recovered);

    Assert.assertTrue(recovered,
        "disabling chi on the poisoned nodes must relocate it to a spare; it did not. "
