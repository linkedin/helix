...
public class TestWorkflowDispatcherStaleCacheFix {
  ...
  /**
   * MessageChange event with stale DELETE prevents deletion and triggers refresh.
   * Simulates the production bug scenario where MessageChange arrives after delete/recreate.
   */
  @Test
  public void testMessageChangeWithStaleDELETEPreventsErroneousDeletion() throws Exception {
    String workflowName = "WorkflowMessageChange";
    ...
    when(context.getFinishTime()).thenReturn(-1L);
    when(context.getJobStates()).thenReturn(Collections.emptyMap());

    dispatcher.updateWorkflowStatus(workflowName, staleDeleteConfig, context,
        currentStateOutput, bestPossibleOutput);

    // Verify: Fresh read was performed to validate DELETE
    verify(dispatcher, times(1)).getFreshWorkflowConfig(workflowName);

    // Verify: requireFullRefresh was called due to stale DELETE detection
    verify(clusterDataCache, times(1)).requireFullRefresh();

    // Verify: Deletion was PREVENTED (workflow not deleted)
    verify(accessor, never()).removeProperty(any(PropertyKey.class));
  }

  /**
   * ResourceConfigChange event with fresh DELETE proceeds with deletion.
   * Shows that when cache is fresh (RESOURCE_CONFIG change), deletion works normally.
   */
  @Test
  public void testResourceConfigChangeWithFreshDELETEProceedsWithDeletion() throws Exception {
    String workflowName = "WorkflowResourceConfigChange";

    WorkflowConfig freshDeleteConfig = mock(WorkflowConfig.class);
    when(freshDeleteConfig.getTargetState()).thenReturn(TargetState.DELETE);
    when(freshDeleteConfig.isTerminable()).thenReturn(true);
    when(freshDeleteConfig.getJobDag()).thenReturn(mock(JobDag.class));
    when(freshDeleteConfig.getJobDag().getAllNodes()).thenReturn(Collections.emptySet());

    doReturn(freshDeleteConfig).when(dispatcher).getFreshWorkflowConfig(workflowName);
    when(clusterDataCache.getWorkflowConfig(workflowName)).thenReturn(freshDeleteConfig);

    WorkflowContext context = mock(WorkflowContext.class);
    when(context.getFinishTime()).thenReturn(System.currentTimeMillis());
    when(context.getJobStates()).thenReturn(Collections.emptyMap());
    ...
    dispatcher.updateWorkflowStatus(workflowName, freshDeleteConfig, context,
        currentStateOutput, bestPossibleOutput);

    // Verify: Fresh read was performed (our fix always validates DELETE)
    verify(dispatcher, times(1)).getFreshWorkflowConfig(workflowName);

    // Verify: Deletion PROCEEDED (both states confirmed DELETE)
    verify(accessor, atLeastOnce()).removeProperty(any(PropertyKey.class));

    // Verify: No requireFullRefresh needed (cache was already fresh)
    verify(clusterDataCache, never()).requireFullRefresh();
  }
}