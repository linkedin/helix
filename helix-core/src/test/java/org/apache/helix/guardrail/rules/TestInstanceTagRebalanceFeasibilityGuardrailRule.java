...
public class TestInstanceTagRebalanceFeasibilityGuardrailRule {
  ...
  public void testGuardrailDisabledByDefaultShortCircuits() {
    // Opt-in and disabled by default: a fresh ClusterConfig leaves the flag unset, so the rule returns
    // feasible before reading the instance config or running the what-if. Both are stubbed to fail so a
    // regression that reaches them is caught.
  ...
  private static ClusterConfig enabledClusterConfig() {
    ClusterConfig clusterConfig = new ClusterConfig(CLUSTER);
    // Opt-in and disabled by default; enable it explicitly for the simulation-path tests.
