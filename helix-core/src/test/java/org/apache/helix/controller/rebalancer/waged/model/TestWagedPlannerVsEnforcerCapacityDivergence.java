...
public class TestWagedPlannerVsEnforcerCapacityDivergence extends AbstractTestClusterModel {
  ...
  private ResourceControllerDataProvider buildCache() throws IOException {
    ...
    cache.getAssignableInstanceConfigMap().get(_testInstanceId).setDomain("zone=" + _testInstanceId);
  ...
  /**
   * Feed the identical fact -- one replica on {@code _testInstanceId} for {@code Partition1} in the
   * transient state {@code INDEX_DOWNLOADED} -- to both ledgers, and show they disagree about how
   * much capacity is left on that node. This is the divergence itself, isolated from any rebalance
   * pipeline: the planner reports the node as untouched and re-proposes the partition, while the
   * enforcer has already charged the node for it and will veto that proposal. Neither side is
   * wrong by its own definition, which is why the loop never terminates.
   */
  @Test
