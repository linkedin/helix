...
public class TestMaxCapacityUsageInstanceConstraint {
  ...
  @Test
  public void testProjectedUsageAboveClusterEstimate() {
    Map<String, Integer> usage = ImmutableMap.of("CPU", 40, "DISK", 90);
    when(_testReplica.getCapacity()).thenReturn(usage);
    when(_testNode.getGeneralProjectedHighestUtilization(eq(usage))).thenReturn(0.9f);
    when(_clusterContext.getEstimatedMaxUtilization()).thenReturn(0.5f);
    ...
    verify(_testNode, times(2)).getGeneralProjectedHighestUtilization(usage);
  }
}
