...
public class TestAssignableNode extends AbstractTestClusterModel {
  ...
  public void testNormalUsage() throws IOException {
    ...
    AssignableReplica removingReplica = new AssignableReplica(testCache.getClusterConfig(),
        testCache.getResourceConfig(_resourceNames.get(1)), new IdealState(_resourceNames.get(1)),
  ...
  public void testAssignDuplicateReplica() throws IOException {
    ...
    AssignableReplica duplicateReplica = new AssignableReplica(testCache.getClusterConfig(),
        testCache.getResourceConfig(_resourceNames.get(0)), new IdealState(_resourceNames.get(0)),
  ...
  @Test(expectedExceptions = HelixException.class, expectedExceptionsMessageRegExp = "The required capacity keys: \\[.*(item1|item2|item3|AdditionalCapacityKey).*(item1|item2|item3|AdditionalCapacityKey).*(item1|item2|item3|AdditionalCapacityKey).*(item1|item2|item3|AdditionalCapacityKey).*\\] are not fully configured in the instance: testInstanceId, capacity map: \\{.*(item1=20|item2=40|item3=30).*(item1=20|item2=40|item3=30).*(item1=20|item2=40|item3=30).*\\}\\.")
